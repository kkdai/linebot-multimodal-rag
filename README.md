...
| 點選 **「📥 存入資料庫」** | 非同步建立索引，完成後推播通知 |
| 點選 **「🔍 作為搜尋」** | 以該圖片 / 文件作為查詢，從資料庫找相關內容 |
| 輸入 **文字** | 直接對資料庫做語意查詢（RAG） |
...
### 搜尋能力
- **文字查文字**：輸入問題 → 找 PDF / 文件中的相關段落
- **圖片查資料庫**：傳圖片 → Gemini 理解圖片內容 → 找資料庫中的相關資訊（包含 PDF 內的圖文說明）
...
- **Generation model**：`gemini-2.0-flash`（可透過環境變數調整）
